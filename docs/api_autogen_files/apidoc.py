...
"""
    ...
    .. warning:: Module references are generated with apigen in pySPACE!

    .. warning:: This version of sphinx code was adapted for pySPACE!
"""
import os
import sys
import optparse
...
def create_module_file(package, module, opts):
    ...
    .. note::
        Currently only used at the top level. 
        The rest is done with apigen module.
        For our framework there are no modules at the top level
    """
    text = '.. AUTO-GENERATED FILE -- DO NOT EDIT! (apidoc module)\n\n'
...
def create_package_file(root, master_package, subroot, py_files, opts, subs):
    ...
    text = '.. AUTO-GENERATED FILE -- DO NOT EDIT! (apidoc package)\n\n'
    text += format_heading(1, '%s Package' % package)
    module_text = format_heading(2, 'Modules')
    module_text += '.. autosummary::\n'
    module_text += '    :toctree:\n\n'
    ...
    for py_file in py_files:
        ...
        if is_package:
            ...
            # if there is just one module, we don't need an extra module file
            if len(py_files)>1 and opts.module:
                module_root=makename(master_package, subroot)
                # create_module_file(module_root,module,opts)
                module_text += '    %s\n'%module#%s.%s\n' % (module_root, module)
                continue
            heading = ':mod:`%s` Module' % py_file
        text += format_heading(2, heading)
        # Package documentation gets __init__ additionally to the package name
        # though it is not needed. This causes problems in combination with
        # autosummary.
        if py_path=="__init__":
            py_path=""
    ...
    if subs:
        toc_text = format_heading(2, 'Subpackages Tree')
        sum_text = format_heading(2, 'Subpackages Summary')
        sum_text += '.. currentmodule:: %s\n'%makename(master_package,subroot)
        sum_text += '.. autosummary::\n\n'
        toc_text += '.. toctree::\n'
        toc_text += '    :maxdepth: %s\n\n' % opts.maxdepth
        for sub in subs:
...
def recurse_tree(rootpath, excludes, opts):
    ...
    for root, subs, files in os.walk(rootpath):
        ...
        if is_pkg:
            ...
            for py_file in py_files:
                if not shall_skip(path.join(root, py_file)):
                    module = path.splitext(py_file)[0]
...
Note: By default this script will not overwrite already created files.""")
    ...
    parser.add_option('-M', '--single-module-page', action='store_true', dest='module',
                      help='One extra page for each module')
    parser.add_option('-q', '--quite', action='store_true', dest='quiet',
                      help='Quiet mode', default=False)
    parser.add_option('-P', '--package-summary', action='store_true', dest='packagesummary',
                      help='Auto summary of sub packages additionally to toctree')
